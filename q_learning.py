...
def check_ghost(blinky_x,blinky_y,pinky_x,pinky_y,player_x,player_y):
    ...
    if abs(pos_player[0] - pos_blinky[0]) <= dist1 or abs(pos_player[0] - pos_pinky[0]) <= dist1 :
        if  (pos_blinky[1] - pos_player[1] <= dist2 and pos_blinky[1] - pos_player[1] >= 0 ) or (pos_pinky[1] - pos_player[1] <=dist2 and pos_pinky[1] - pos_player[1] >= 0) :
            #  print(pos_pinky[1] - pos_player[1])
             check[0] = True
        if ( pos_player[1] - pos_blinky[1] <= dist2 and pos_player[1] - pos_blinky[1] >= 0 )or (pos_player[1] - pos_pinky[1] <= dist2  and pos_player[1] - pos_pinky[1] >= 0):
            #  print(pos_player[1] - pos_pinky[1])
             check[1] = True
    if abs(pos_player[1] - pos_blinky[1]) <= dist1 or abs(pos_player[1] - pos_pinky[1]) <= dist1 :
        if (pos_blinky[0] - pos_player[0] <=dist2 and 0 < pos_blinky[0] - pos_player[0] >= 0) or  (pos_pinky[0] - pos_player[0] <=dist2 and pos_pinky[0] - pos_player[0] >= 0) :
             check[3] = True
        if (pos_player[0] - pos_blinky[0] <= dist2 and pos_player[0] - pos_blinky[0] >= 0) or (pos_player[0] - pos_pinky[0] <= dist2 and pos_player[0] - pos_pinky[0] >= 0) :
             check[2] = True
    return check
